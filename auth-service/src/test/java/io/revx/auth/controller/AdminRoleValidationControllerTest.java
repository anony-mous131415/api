...
public class AdminRoleValidationControllerTest {
  ...
  /**
   * Test method for {@link io.revx.auth.controller.AdminRoleValidationController#checkNoRole()}.
   */
  @Test
  public void testCheckNoRole() throws Exception {
    ResponseEntity<ApiResponseObject<String>> resp = adminRoleValidationController.checkNoRole();
    assertNotNull(resp);
    assertNotNull(resp.getBody());
    assertNotNull(resp.getBody().getRespObject());
    assertThat(resp.getBody().getRespObject()).contains("checkNoRole");
  }
