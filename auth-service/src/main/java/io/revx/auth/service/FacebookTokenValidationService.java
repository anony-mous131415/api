...
public class FacebookTokenValidationService implements ExternalTokenValidationService {
  private static String FACEBOOK_EMAIL_KEY = "email";

  private static Logger logger = LogManager.getLogger(FacebookTokenValidationService.class);
  @Autowired
  RestTemplate restTemplate;
  @Autowired
  SecurityConstants securityConstants;

  @LogMetrics(name = GraphiteConstants.SERVICE + GraphiteConstants.SOCIAL + ".fb")
  @Override
  public UserInfoModel validateToken(String token) throws UserUnAuthenticateException {
    logger.debug("FacebookTokenValidationService  validateToken :" + token);
    StringBuffer sb =
        new StringBuffer(securityConstants.getFACEBOOK_AUTH_API()).append(StringUtils.trim(token));
  ...
  private String getUserName(Map map) {
    return (String) map.get(FACEBOOK_EMAIL_KEY);
  }
}
