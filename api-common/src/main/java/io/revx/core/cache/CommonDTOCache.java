...
public class CommonDTOCache<T> extends CommonCache<T> {

	private static final Logger logger = LogManager.getLogger(CreativeDTO.class);

	private Map<String, String> fieldsAndMethodMap = null;

	public CommonDTOCache(String name, Class<?> classTypeToCache) {
		if (StringUtils.isBlank(name))
			throw new NullPointerException("name cannot be null or empty");
		fieldsAndMethodMap = BeanUtils.getPropertyAndMethodMap(classTypeToCache);
		// TODO: HArdcoding here for status because status is Inside Object
	...
	public CommonDTOCache(String name, Class<?> classTypeToCache, Long timeToExpire) {
	...
	private Cache getCache(Class<?> classTypeToCache) {
	...
	public List<T> getAll(List<String> sortOn) throws UnsortableAttributeException {
		if (!cacheAvailable())
			return null;
	...
	public List<T> getAll(Set<DashboardFilters> filters, List<String> sortOn)
	...
	public List<T> getAll(String filter, List<String> sortOn, String dataType)
			throws UnsortableAttributeException {
		if (!cacheAvailable())
			return null;
	...
	public void populate(Collection<T> data) {
		if (data == null) {
			return;
		}
		...
		for (T b : data) {
			logger.debug("--> Adding to cache {} : {} ", ((BaseEntity) b).getId(), b);
			Element element = new Element(((BaseEntity) b).getId(), b);
			element.setTimeToLive(timeToLive);
			cache.put(element);
		}
	}

	public void populate(BaseModel data) {
		if (data == null) {
			return;
		}
		int timeToLive = (int) (timeToExpire/1000);
	...
	public void populate(Integer id, BaseModel data) {
		if (data == null) {
			return;
		}
		int timeToLive = (int) (timeToExpire/1000);
