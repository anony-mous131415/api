...
PROG="io.revx.api.ApiMainApplication"
...
SERVICE_HOME=/atom/api-service
PID_FILE=$SERVICE_HOME/apiService.pid
LOCKFILE=$SERVICE_HOME/apiService.lock
STOP_TIMEOUT=3
STATUS_FILE=$SERVICE_HOME/status/ztstatus.txt
SLEEP_SECONDS=5
RETVAL=0
JAVA_OPTS="-server -Xms128m -Xmx512m -XX:+UseG1GC -javaagent:/atom/dd-java-agent.jar -Ddd.profiling.enabled=true -Ddd.logs.injection=true -Ddd.service=api-service javaagent:/atom/api-service/current/lib/org.jacoco.agent-0.8.5-runtime.jar=address=*,port=9000,dumponexit=false,output=tcpserver"
...
start() {
        ...
        if [[ $PID -ne 0 ]] || [[ $PROCESS_COUNT -ge 1 ]];then 
                ...
                #echo $CP
                sudo -u atomex  $JAVA_HOME/bin/java  $JAVA_OPTS -cp $CP -Djasypt.encryptor.password=mySecretKey@123 $PROG > $SERVICE_HOME/logs/apiServiceConsoleOutput.log 2>&1&
                sleep 5;
                PID=$(${JAVA_HOME}/bin/jps -lV | awk "/$PROG/{print \$1}")
                RETVAL=$?
                sleep 3
